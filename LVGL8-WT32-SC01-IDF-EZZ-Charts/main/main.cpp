...
static void update_chart_timer_cb(struct _lv_timer_t * timer);
char txt[100];

int temp,temp_new,humi,humi_new,randv_new,randv,temp_old,humi_old,randv_old;
...
extern "C"
{
    void app_main(void)
    {
        ...
        ESP_ERROR_CHECK(esp_timer_start_periodic(periodic_timer, LV_TICK_PERIOD_MS * 1000));
		    ...
		    // Initialize the UI exported from EEZ Studio
		ui_init();
		
		    // Create a periodic LVGL timer (every 50ms)
		lv_timer_create(update_chart_timer_cb, 50, NULL);
...
static int smooth(int new_val, int old_val)
{
    return (old_val * 7 + new_val * 3) / 10; // weighted smoothing
}

static void update_chart_timer_cb(struct _lv_timer_t * timer) {
	
    temp_new = esp_random() % 40 + 20;
    humi_new = esp_random() % 40 + 40;
    randv_new = esp_random() % 100;
	
	temp = smooth(temp_new,temp_old);
	humi = smooth(humi_new,humi_old);
	randv = smooth(randv_new,randv_old);
    ...
    lv_chart_set_next_value(objects.ch, objects.ch_series_temp, temp);
    lv_chart_set_next_value(objects.ch, objects.ch_series_humi, humi);
    lv_chart_set_next_value(objects.ch, objects.ch_series_rand, randv);
	
	    // --- UPDATE THE 3 BARS BASED ON CURRENT VALUES ---
    lv_bar_set_value(objects.bar_temp, temp, LV_ANIM_ON);
    lv_bar_set_value(objects.bar_humi, humi, LV_ANIM_ON);
    lv_bar_set_value(objects.bar_rand, randv, LV_ANIM_ON);

    // needed for redraw
    lv_chart_refresh(objects.ch);
}
